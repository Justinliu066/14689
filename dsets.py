...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(path+'dataset//subset*//*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open('E:\\pytorch_even\\data\\part2\\luna\\annotations_with_malignancy.csv') as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
            annotationCenter_xyz = tuple([float(x) for x in row[1:4]])
            annotationDiameter_mm = float(row[4])
            # 处理数值格式的恶性标记
            mal_value = row[5].strip()
            if mal_value in {'True', '1', '1.0'}:
                isMal_bool = True
            else:
                isMal_bool = False
...
class Ct:
    def __init__(self, series_uid):
        # 修复文件路径匹配逻辑
        mhd_pattern = path + f'dataset//subset*//{series_uid}.mhd'
        mhd_path_list = glob.glob(mhd_pattern)
        
        if not mhd_path_list:
            raise ValueError(f"没有找到系列 {series_uid} 的CT数据，路径模式: {mhd_pattern}")
    ...
    def getRawCandidate(self, center_xyz, width_irc):
        ...
        for axis, center_val in enumerate(center_irc):
            ...
            if center_val < 0:
                log.warning(f"中心坐标超出范围: axis={axis}, center_val={center_val}, series_uid={self.series_uid}")
                center_val = 0
                start_ndx = 0
                end_ndx = int(width_irc[axis])
            
            if center_val >= self.hu_a.shape[axis]:
                log.warning(f"中心坐标超出范围: axis={axis}, center_val={center_val}, shape={self.hu_a.shape[axis]}, series_uid={self.series_uid}")
                center_val = self.hu_a.shape[axis] - 1
...
class MalignantLunaDataset(LunaDataset):
    def __len__(self):
        if self.ratio_int:
            ...
            return len(self.ben_list) + len(self.mal_list)
